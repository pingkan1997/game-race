	...
	public :
		...
		void gores()
		{
			for(int i = y; i<= (y + 3); i++)
			{
				...
				{
					if(gambar[i - y][i] == '0')cout<<char(221); else
					if(gambar[i - y][i] == '1')cout<<char(222); else
					if(gambar[i - y][i] == '+')cout<<BATANG; else
					if(gambar[i - y][i] == '-')cout<<char(220); else
					cout<<" ";
				}
			}
		}
...
int main()
{
	...
	{
		system("cls");
		tulisTengah("RACED CAR by bubu gaming v 1.0", 10);
		tulisTengah("==================================", 11);
		tulisTengah("Masukkan Tingkat Kesulitan", 13);
		tulisTengah("Semakin Besar Nilai Maka Semakin Mudah", 14);
		tulisTengah("--			BUBU PROGRAMMING			--", 20);
	...
	tulisTengah("Bubu Gaming", 18);
	tulisTengah("Created 2022", 19);
